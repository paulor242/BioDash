...
class BioDashCollector:
    ...
    def generate_fake_data(self):
        # Formato CSV para YoyoSQ/Polea: acc_avg,acc_max,... vel_max ~10 campos+
        values = [round(random.uniform(8,15),2) for _ in range(12)]
        csv_data = ','.join(map(str, values))
        # Parse como real data
        data = {
            'acceleration_avg': values[0],
            'acceleration_max': values[1],
            'exentric_power_max': values[2],
            'exentric_power_avg': values[3],
            'concentric_power_max': values[4],
            'concentric_power_avg': values[5],
            'concentric_force_max': values[6],
            'concentric_force_avg': values[7],
            'velocity_avg': values[8],
            'velocity_max': values[9],
            'exentric_force_max': values[10],
            'exentric_force_avg': values[11]
        }
        print(f"🧪 FAKE DATA: {csv_data} → {data}")
        return data
    ...
    def run(self):
        ...
        # Mostrar puertos (solo real mode)
        ...
        try:
            while time.time() - start_time < timeout and reading_count < max_readings:
                ...
                if data:
                    ...
                    print(f"⚠️ No data in this interval, retrying...")
                    continue
